...
public class SetService {
	//Set(집합)
	// - 순서를 유지하지 않음(==인덱스 없기때문에)
	// - 중복을 허용하지 않는다. ( + null도 중복x 1개만 저장 가능)
	//주머니에 손넣어서 암거나 꺼내는것 == set
	
	
	//★//****set이 중복을 확인하는 방법*****
	//컬렉션은 객체만 저장할 수 있음.(set도 컬렉션이기 때문에 객체만 저장)
	...
	public void ex1() {
		...
		//사용 조건2 : 저장되는 객체 hashCode() 오버라이딩 필수
		//String 은 굉장히 완성도 높은 클래스로,
		//이때 equals,hashCode가 오버라이딩 되어있음
		//다른클래스 만들때는 equals,hashCode 오버라이딩 해야함.
		...
		//list는 인덱스가 있어서 순서유지도 되고 그렇지만 set은 인덱스가 없기때문에
		//순서유지도 안됨 중복도 그렇고~ 그래서 스트링값 자체를 써줘야함
		
		System.out.println(set.remove("해운대"));//삭제하면 true
		...
		while(it.hasNext()) {// ->it의 꺼내올 다음 값이 있는지?
			//while문을 쓴 이유는 반복할 횟수를 몰라서 = 인덱스가 없으니까.
	...
	public void ex2() {
		...
		// hash 함수 : 입력된 단어를 고정된 길이의 문자열 반환하는 함수 (중복X)
		// ex) 입력 111 -> "fdsgd" (5글자)
		// ex) 입력 123456789 -> "qwert" (5글자)
		// =>암호화에 사용
		// 일정한 길이의 랜덤값 생성, 중복되지 않음
		
		//hashCode() : 필드 값이 다르면 중복되지 않은 숫자를 만드는 메서드
	...
	//	
		if(mem1.getId().equals(mem2.getId())) {
			if(mem1.getPw().equals(mem2.getPw()))  {
				if(mem1.getAge()==(mem2.getAge()))  {
					System.out.println("같은객체입니다 (true)");
				}
			}
		}
		...
		// ===> equals 메소드 오버라이딩
		System.out.println("=================");
		System.out.println(mem1.equals(mem2));//mem1 mem2 필드같은가
		System.out.println(mem1.equals(mem3));//mem1 mem3 필드같은가
	...
	public void ex3() {
		...
		memberSet.add(new Member("user01","passe01",30) );
		memberSet.add(new Member("user02","passe02",40) );
		memberSet.add(new Member("user03","passe03",20) );
		memberSet.add(new Member("user04","passe04",25) );
		memberSet.add(new Member("user04","passe04",25) );
		...
		Member mem1 = new Member("user01","pass01",30);
		Member mem2 = new Member("user01","pass01",30);
		Member mem3 = new Member("user02","pass01",20);
	...
	public void ex4() {
		...
		//기본 자료형 -> 포장
		Integer w1 = new Integer(iNum);//이제 안쓰는거라 권하지않음으로 삭제선 나옴.
	...
	public void ex5() {
		...
		Integer w2 = 100;
		Integer w3 = 200;
	//	(Integer) (int -> Integer) 자료포장
		// AutoBoxing
		//w2와 100은 원래 연산이 안되어야 하지만
		//Integer는 int 의 포장형식이라는걸 Java가 인식하고 있음
	...
	//	Set<Integer> lotto = new LinkedHashSet<Integer>();// 순서유지 sey
		Set<Integer> lotto = new TreeSet<Integer>();
		//Integer는 equals(),hashCode(),오버라이딩 완료 상태
		
		while(lotto.size() < 6) {
			//로또의 저장된 값이 갯수가 6개 미만이면 반복
		 int random = (int)(Math.random()* 45 +1);//1~ 45 사이 난수
		 
		System.out.println(random);//발생 순서 확인
		 lotto.add(random); 
		 //int값이 자동으로 Integer로 포장 ( AutoBoxing )eh되어 lotto추가
